...
TGAImage *img = new TGAImage(winw, winh);
...
mjrContext con;
float depth_buffer[winw*winh];        // big enough for 5K screen
unsigned char depth_rgb[winw*winh*3];
unsigned char depth_rgb_backup[winw*winh * 3];
...
void autoscale(GLFWwindow* window)
{
	...
	cam.azimuth = 90;
	cam.elevation = 0;
    cam.distance = 1.2 * m->stat.extent;
...
void render(GLFWwindow* window)
{
	...
	if (outputToFile) {
		...
		mjr_readPixels(depth_rgb_backup, NULL, rect, &con);
		
		//declare a temporary color variable
		Colour c;
		
		//Loop through image and set all pixels to framebuffer value
		for (int x = 0; x<winh; x++)
			for (int y = 0; y<winw; y++)
			{
				...
				c.r = depth_rgb_backup[3 * x_transf * winw + 3 * y_transf + 0];
				c.g = depth_rgb_backup[3 * x_transf * winw + 3 * y_transf + 1];
				c.b = depth_rgb_backup[3 * x_transf * winw + 3 * y_transf + 2];
				c.a = 255;
		...
		string fp = "E:\\Google Drive\\Github\\tempdata\\Biomechanical Model\\figures\\mujoco_output.tga";
...
int main(int argc, const char** argv)
{
    ...
    while( !glfwWindowShouldClose(window) && !force_close)
    {
        // simulate and render
        render(window);
		...
		// Uncomment below to run only one frame
		force_close = true;
    }
