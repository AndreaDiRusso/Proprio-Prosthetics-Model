...
mujocoVideoName = 'mujoco_video.avi'
...
y1a = 0
x2a = aw - 300 # botom right coordinates
y2a = ah
aw_prime = x2a - x1a
ah_prime = y2a - y1a

x1m = 1000 # top left coordinates
y1m = 100
x2m = mw-100 # botom right coordinates
y2m = mh-200
mw_prime = x2m - x1m
mh_prime = y2m - y1m
...
outAnimal = cv2.VideoWriter(animalVideoPathOut,fourcc, fps, (1500,750), isColor=True)
outMujoco = cv2.VideoWriter(mujocoVideoPathOut,fourcc, fps, (1500,750), isColor=True)
...
while cv2.waitKey(1) != ord(' ') and animalVideo.isOpened() and mujocoVideo.isOpened():  # until the Esc key is pressed or webcam connection is lost
    ...
    imgAnimal = cv2.resize(imgAnimal, (1500,750))
    imgMujoco = cv2.resize(imgMujoco, (1500,750))
